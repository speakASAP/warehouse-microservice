...
`[MISSING: owner-approved Warehouse stock hold/release window and max quantity]` is narrowed only for candidate max quantity because Catalog source-documents max hold quantity `1` per component for the candidate packet. Warehouse source has `DEFAULT_RESERVATION_TTL_MS = 15 * 60 * 1000` and accepts an explicit `expiresAt`, but that is implementation behavior, not owner approval for a live canary window, hold/release duration, timeout owner, or mutation window. [MISSING: renewed owner-approved execution window and Warehouse hold/release duration]; [MISSING: final owner approval before any live Warehouse reservation/cleanup mutation].
