...
| Stock hold window | Commerce/Warehouse owner | maximum hold duration, expiry owner, cleanup owner if the smoke aborts before fulfillment, and whether timeout cleanup uses Warehouse expiry or explicit release | `[MISSING: renewed owner-approved execution window and Warehouse hold/release duration]` |
...
| Timeout-state cleanup owner | Commerce/Warehouse owner | owner-approved choice between TTL/expiry-owned `expire` and explicit abort-owned `release` for active holds | `[RESOLVED/NARROWED: Warehouse source operation is defined; live owner selection remains missing]`; `[MISSING: renewed owner-approved execution window and Warehouse hold/release duration]` |
