...
- Goal Impact: addresses the Warehouse-owned part of `[MISSING: owner-approved Warehouse stock hold/release window and max quantity]` and `[MISSING: owner-approved post-fulfillment cancellation/return workflow that maps a Payments refund or correction to Orders and Warehouse without inferring stock effects]` as far as source/docs can safely prove.
...
`[MISSING: owner-approved Warehouse stock hold/release window and max quantity]` remains unresolved. Warehouse source proves how a hold is released, fulfilled, cancelled, or returned, but it does not approve a live stock window, target stock rows, maximum quantity, target order ids, timeout/TTL override, or rollback owner for a paid/provider canary.
...
| Stock hold window | Commerce/Warehouse owner | maximum hold duration, expiry owner, and cleanup owner if the smoke aborts before fulfillment | `[MISSING: owner-approved Warehouse stock hold/release window and max quantity]` |
...
- Do not create a live paid/provider bundle smoke if max quantity, hold window, target component stock rows, or cleanup owner is missing.
...
| --- | --- | --- | --- | --- | --- | --- |
| Warehouse cleanup approval packet | complete-source-docs | Warehouse reservation owner | this packet and static verifier | existing Warehouse lifecycle source evidence | verifier, focused tests, build, diff check | Ready for owner review; no runtime permission granted. |
