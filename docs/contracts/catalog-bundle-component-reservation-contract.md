...
- State Update: `[RESOLVED: Warehouse approval that first ecosystem bundle selling reserves component lines only]`; `[RESOLVED/NARROWED: Warehouse owner-approved cleanup operation for reserved-only, fulfilled/stock-decremented, return, partial component failure, and timeout component-line states; candidate max quantity is source-documented from Catalog packet, while exact selected reservation lookup state remains missing]`; `[MISSING: owner-approved paid/provider checkout smoke with stock and refund/cancel rollback plan]` remains blocked beyond existing pending-order reservation/release evidence.
...
| Unknown, ambiguous, duplicate, or missing component reservation state | none; fail closed | A deterministic component reservation lookup is missing | no mutation | Orders/integration must stop before the next side effect and record `[MISSING: exact selected Warehouse reservation lookup state for cleanup]`. |

## Deterministic Cleanup Packet

Warehouse source-policy now defines the cleanup packet shape for future paid/provider validation. The packet must be line-based, not bundle-based, and must contain one row for each component reservation with `bundleContractVersion` as audit evidence only, plus `orderId`, `channel`, `productId`, `warehouseId`, `quantity`, optional `reservationId`, `currentReservationStatus`, `approvedCleanupOperation`, `cleanupReasonCode`, and `actor`. The read-only source of truth for runtime state is `GET /api/reservations/order/:orderId`, which returns Warehouse reservation rows carrying `id`, `productId`, `warehouseId`, `quantity`, `orderId`, `channel`, and `status`.

A cleanup packet is deterministic only when each component line resolves exactly one Warehouse reservation by `orderId + channel + productId + warehouseId + quantity` and, when present, `reservationId`. `active` rows map to `release` or TTL-owned `expire`; `fulfilled` rows map to `cancel` or `return` only after the approved external business event; `released`, `cancelled`, `returned`, and `expired` rows map to `none` unless the owner-approved packet asks for idempotency proof. Zero, duplicate, mixed-key, quantity-mismatched, or aggregate bundle rows fail closed before any mutation.

Result: `[RESOLVED/NARROWED: deterministic Warehouse component-line cleanup packet for reserved-only, fulfilled, cancel, return, partial failure, and timeout states]`. Candidate target component stock rows and max component quantity are source-documented from the Catalog packet: `[RESOLVED/NARROWED: candidate target component stock rows and max component quantity are source-documented from Catalog packet]`. Runtime remains blocked by `[RESOLVED/NARROWED: live current target row readback at execution time captured through protected Warehouse API without mutation]`, `[RESOLVED/NARROWED: Warehouse hold/release duration is owner-approved for the bounded Goal 24 smoke as 15 minutes source-default TTL or shorter caller-supplied expiresAt]; [MISSING: exact selected Warehouse reservation lookup state for cleanup]`, and `[RESOLVED/NARROWED: final owner approval before live Warehouse reservation mutation is bounded to one Goal 24 component-line smoke attempt with max quantity 1 per component after live readback]`.
...
- use no Warehouse operation when the state is unknown; fail closed and require a deterministic component-line cleanup packet plus a new owner-approved runtime packet.

This resolves/narrows the operation-selection portion of `[MISSING: owner-approved operation for reserved-only, fulfilled/stock-decremented, return, partial component failure, and timeout states, including max quantity and hold/release window]`: Warehouse source-policy defines `release`, `cancel`, `return`, line-by-line partial cleanup, and timeout/expiry behavior, and Catalog source-documents candidate target component rows plus max hold quantity `1` per component. Runtime remains blocked because exact provider proof, target order hash/state, named actor, side-effect acknowledgements, live Warehouse row readback, renewed execution window/hold duration, deterministic cleanup lookup, and final mutation approval are still missing.
